...
eulerStutterStep(REALfour *state, int tr[5])
{
    ...
    //Add the change back to the node in question.
    #ifdef __CUDA_ARCH__
    state[tr[2]] += FOURVEC(0.5 * dimens.x * (fluxL-fluxR));
    #else
    state[tr[2]] += FOURVEC(0.5 * dimz.x * (fluxL-fluxR));
    #endif
    state[tr[2]].w = pressure(state[tr[2]]);
...
upTriangle(const REALfour *IC, REALfour *right, REALfour *left)
{
	...
	for (int k = -2; k<3; k++)
    {
        ...
        tid_top[k+2] = tididx + k + geo.y;
    }

	int leftidx = ((tid/4 & 1) * geo.y) + (tid/4)*2 + (tid & 3) + 2;
	int rightidx = (geo.y-6) + ((tid/4 & 1) * geo.y) + (tid & 3) - (tid/4)*2;
        ...
        gid += blockDim.x*gridDim.x)
    {
    	...
    	for (int k = 4; k<(blockDim.x/2); k+=4)
    	{
    		if (tid < (blockDim.x-k) && tid >= k)
    		{
                ...
                temper[tididx].w = pressure(temper[tid]);
    		}
