...
__forceinline__
void
readIn(REAL *temp, const REAL *rights, const REAL *lefts, int td, int gd, int adj)
{
	int leftidx = disc.ht + (((td>>2) & 1) * disc.base) + (td & 3) - (4 + ((td>>2)<<1));
	int rightidx = disc.ht + (((td>>2) & 1) * disc.base) + ((td>>2)<<1) + (td & 3);

	temp[leftidx+adj] = rights[gd];
	temp[rightidx+adj] = lefts[gd];
}

__device__
__forceinline__
void
writeOut(REAL *temp, REAL *rights, REAL *lefts, int td, int gd, int adj)
{

    int leftidx = (((td>>2) & 1)  * disc.base) + ((td>>2)<<1) + (td & 3) + 2; //left get
    int rightidx = (disc.base-6) + (((td>>2) & 1)  * disc.base) + (td & 3) - ((td>>2)<<1); //right get

	rights[gd] = temp[rightidx+adj];
	lefts[gd] = temp[leftidx+adj];
...
__device__
__forceinline__
REAL stutterStep(REAL *state, int td)
{
	return state[td] - disc.dt_half * (convect(state[td-1], state[td+1]) + secondDer(state[td-1], state[td+1], state[td]) +
		fourthDer(state[td-2], state[td-1], state[td], state[td+1], state[td+2]));
}

__device__
__forceinline__
REAL finalStep(REAL *state, int td)
{
	return (-disc.dt * (convect(state[td-1], state[td+1]) + secondDer(state[td-1], state[td+1], state[td]) +
		fourthDer(state[td-2], state[td-1], state[td], state[td+1], state[td+2])));
}

__device__
__forceinline__
REAL stutterStepClassic(REAL tfarLeft, REAL tLeft, REAL tCenter, REAL tRight, REAL tfarRight)
{
...
__device__
__forceinline__
REAL finalStepClassic(REAL tfarLeft, REAL tLeft, REAL tCenter, REAL tRight, REAL tfarRight)
{
...
{
    int gid = blockDim.x * blockIdx.x + threadIdx.x; //Global Thread ID
    int lastidx = disc.idxend;

	if (final)
	{
		ks_out[gid] += finalStepClassic(ks_in[(gid-2)&lastidx], ks_in[(gid-1)&lastidx],
			ks_in[gid], ks_in[(gid+1)&lastidx], ks_in[(gid+2)&lastidx]);
	}
	else
	{
		ks_out[gid] = stutterStepClassic(ks_in[(gid-2)&lastidx], ks_in[(gid-1)&lastidx], ks_in[gid],
...
upTriangle(const REAL *IC, REAL *right, REAL *left)
{
    ...
    int tididx = tid + 2;
	int tidxTop = tididx + disc.base;
    int tididx2 = tidxTop + disc.base;
    int tidxTop2 = tididx2 + disc.base;
    ...
    temper[tididx2] = IC[gid+disc.idxend_h];

	__syncthreads();

	if (tid > 1 && tid <(blockDim.x-2))
	{
		temper[tidxTop] = stutterStep(temper, tididx);
        temper[tidxTop2] = stutterStep(temper, tididx2);
	}

	__syncthreads();

	//The initial conditions are timslice 0 so start k at 1.
	for (int k = 4; k<(blockDim.x>>1); k+=4)
	{
		if (tid < (blockDim.x-k) && tid >= k)
		{
            temper[tididx] += finalStep(temper, tidxTop);
            temper[tididx2] += finalStep(temper, tidxTop2);
		}
    ...
    writeOut(temper, right, left, tid, gid, 0);
    writeOut(temper, right, left, tid, gid+disc.idxend_h, disc.base2);
}
...
downTriangle(REAL *IC, const REAL *right, const REAL *left)
{
    ...
    readIn(temper, right, left, tid, gid, 0);
    readIn(temper, right, left, tid, gid+disc.idxend_h, disc.base2);
    ...
    IC[gid+disc.idxend_h] = temper[tididx2];
}
	...
	REAL *IC, REAL *T_f, const REAL freq, ofstream &fwr)
{
	...
	const size_t smem = (4*tpb+8)*sizeof(REAL);
	...
	//Split
	wholeDiamond <<< bks,tpb,smem >>> (d_right,d_left);
...
int main( int argc, char *argv[])
{
    ...
    const int bks = dv/(2*tpb); //The number of blocks
